...
(function () {
  ...
  // tunables (driven by Tweaks). simulate=false → statuses come from REAL
  // TWR employee signals (agenthq-data.js sync); toggle on for demo churn.
  window.CFG = { speed: 1, showLabels: true, simulate: false, fxDust: true };
  ...
  // expose for tweaks + live data sync (agenthq-data.js)
  window.SCENE = {
    agents, refreshHUD,
    // push a real status from TWR onto an agent by id
    setAgentStatus(id, patch) {
      const ag = agents.find(a => a.data.id === id);
      if (!ag || !patch) return;
      const oldStatus = ag.data.status;
      if (patch.status && window.STATUS[patch.status]) ag.data.status = patch.status;
      if (patch.task != null)       ag.data.task = patch.task;
      if (patch.progress != null)   ag.data.progress = patch.progress;
      if (patch.tasksToday != null) ag.data.tasksToday = patch.tasksToday;
      if (patch.status && window.STATUS[patch.status].zone !== window.STATUS[oldStatus].zone) reassignSpot(ag);
      refreshHUD();
      if (selected === ag) renderPopup(ag);
    },
  };
